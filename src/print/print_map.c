...
/*   Updated: 2024/01/21 20:00:08 by babonnet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*create_transformation_matrix(double pitch, double yaw, double zoom, double offset_x, double offset_y)
{
	pitch = fmod(pitch, 360);
	yaw = fmod(yaw, 360);
	double	*transformation;
	double	tempMatrix[16];
	double	pitchMatrix[16] = {1, 0, 0, 0, 0, cos(pitch), -sin(pitch), 0, 0,
			sin(pitch), cos(pitch), 0, 0, 0, 0, 1};
	double	yawMatrix[16] = {cos(yaw), 0, sin(yaw), 0, 0, 1, 0, 0, -sin(yaw), 0,
			cos(yaw), 0, 0, 0, 0, 1};
	double	scaleMatrix[16] = {zoom, 0,		 0,		 offset_x,
								0,	 zoom,	 0,		 offset_y,
								0,	 0,		zoom,	 0,
								0,	 0,		 0,		 1};
