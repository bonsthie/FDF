...
/*   Updated: 2024/01/14 23:26:03 by babonnet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	*create_transformation_matrix(float pitch, float yaw, float zoom, float offset_x, float offset_y)
{
	...
	float	yawMatrix[16] = {cos(yaw), 0, sin(yaw), 0, 0, 1, 0, 0, -sin(yaw), 0,
			cos(yaw), 0, 0, 0, 0, 1};
	float	scaleMatrix[16] = {zoom, 0,		 0,		 offset_x,
								0,	 zoom,	 0,		 0,
								0,	 0,		zoom,	 offset_y,
								0,	 0,		 0,		 1};
...
void	create_point2D(float *vect3, int *coord, float *orientation, t_map *map)
{
	float	*m_fov;
	...
	coord[0] = ((m_fov[0] / m_fov[3]) + 1) * (WIDTH / 2.0) + map->offset_x;
	coord[1] = ((m_fov[1] / m_fov[3]) + 1) * ((float)HEIGHT / 2.0) + map->offset_y;
}
...
void	print_map(t_map *map, t_data data)
{
	...
	co_i = 0;
	orientation = create_transformation_matrix(map->pitch, map->yaw, map->zoom, map->offset_x, map->offset_y);
	while (i < size)
	{
		vect3[0] = i % map->width;
		vect3[1] = map->y[i];
